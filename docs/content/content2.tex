...
M2. MediaPipe Pose &
  ...
  min\_detection\_confidence $\geq$ 0.7; extracción de los landmarks 12, 14 y
  16 de Pose (hombro, codo y muñeca derechos) y 0, 4 y 8 de Hands (muñeca,
  punta del pulgar y punta del índice) \\
\midrule
M3. Ángulos &
...
M4. Brazo virtual &
  ...
  Valores continuos $[0^\circ, 180^\circ]$; posibles discontinuidades en límites angulares &
...
cámara. El brazo robótico virtual corresponde a un UR5 con gripper RG2 dentro
de CoppeliaSim; se controlan hombro, codo y apertura binaria del gripper. El
sistema no controla hardware físico. \cite{Vintimilla2026}
...
RF-02 &
  ...
  Pose con una confianza mínima de 0.7, extrayendo los landmarks 12, 14 y 16
  del brazo derecho, y debe asociar la mano correspondiente para utilizar la
  muñeca, punta del pulgar y punta del índice (0, 4 y 8 de Hands) &
  Alta & M2, M3 \\
\midrule
RF-03 &
  El sistema debe calcular los ángulos de hombro y codo con un
  MAE máximo de $5^\circ$ respecto de una referencia independiente &
  Alta & M3 \\
\midrule
RF-04 &
...
\midrule
RF-05 &
  El sistema debe controlar un UR5/RG2 en CoppeliaSim, respetando límites y
  estados de seguridad &
  Alta & M4 \\
\midrule
RF-06 &
  El sistema debe mostrar cámara humana, vista del simulador, ángulos, apertura,
  FPS y estado de seguridad en un dashboard &
  Media & M5 \\
\midrule
RF-07 &
...
\midrule
RNF-02 &
  Precisión angular: error medio (MAE) $\leq 5^\circ$ respecto a medición
  de referencia &
...
\midrule
OpenCV (cv2)  & 4.8+   & Apache 2.0  & Captura, preprocesamiento y anotación visual (M1, M5) \cite{Bradski2000} \\
\midrule
MediaPipe     & 0.10+  & Apache 2.0  & Pose y Hands; extracción del brazo derecho y apertura del gripper (M2, M3) \cite{Bazarevskyetal2020, Lugaresietal2019} \\
\midrule
NumPy         & 1.24+  & BSD         & Álgebra vectorial para cálculo angular y manejo de arrays (M3) \\
\midrule
CoppeliaSim   & 4.10   & Educativa   & Simulación del UR5/RG2, joints, colisiones y Vision Sensor (M4) \\
\midrule
ZeroMQ Remote API & 2.0+ & BSD & Comunicación entre Python y CoppeliaSim (M4) \\
\midrule
...
\subsubsection*{Diseño Preliminar del Módulo M4 -- Cinemática Directa 2D}

El diseño preliminar consideró un brazo robótico virtual como una cadena cinemática de tres
...
Durante la implementación, este diseño fue sustituido por la escena UR5/RG2 de
CoppeliaSim descrita en la Sección~\ref{subsec:implementacion_m4}. Se conserva
esta subsección para documentar la evolución entre el concepto del Sprint 2 y
la solución final.
...
la interfaz de usuario en el dashboard Tkinter, que integra los frames
procesados por OpenCV y la vista del Vision Sensor de CoppeliaSim.
...
los siguientes elementos en un dashboard de escritorio. La cámara humana y la
vista fija de CoppeliaSim aparecen en paneles del mismo tamaño; debajo se ubican
la telemetría, el diagnóstico y la leyenda de atajos de seguridad:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/boceto_pantalla.png}
    \caption{Boceto conceptual preliminar de las fuentes visuales. La
             implementación final las reúne en un único dashboard con paneles
             de igual tamaño, telemetría compacta y operación por teclado.}
...
Sprint \#3 -- Fase 2 & 8--10 &
  M4: integración del UR5/RG2 en CoppeliaSim, comunicación ZeroMQ, límites,
  watchdog, colisiones y ESTOP. M5: dashboard con cámara humana, Vision Sensor,
  telemetría y controles. Integración de M1--M5. &
  Sistema virtual integrado y verificable \\
\midrule
